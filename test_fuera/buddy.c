...
void meminfo(mem_info *memInfo) {
    MemoryManagerADT memoryManager = getMemoryManager();
    memInfo->total = memoryManager->totalMemory;
    memInfo->used = memoryManager->usedMemory;
    memInfo->free = memoryManager->totalMemory - memoryManager->usedMemory;
}
...
static void releaseNode(uint64_t node) {
    MemoryManagerADT memoryManager = getMemoryManager();
    memoryManager->tree[node].use = (memoryManager->tree[node].use == USED) ? 0 : USED;  

    while (node != 0 && memoryManager->tree[((node - 1) ^ 1) + 1].use != USED && memoryManager->tree[((node - 1) ^ 1) + 1].use != SPLIT) {
        node = (node - 1) >> 1;  
